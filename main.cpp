...
/*
please copy lines below each test case
{ Sample program
  in TINY language
...
//adding the new operation (absolute value) to the token types enums
enum TokenType{
...
//adding the new operation to all token types strings
...
//assigning the absolute_difference enum to its operator & in tokens
...
// mathexpr -> absdiffexp { (+|-) absdiffexp }    left associative
// absdiffexp -> term{& term}
...
// absdiffexpr -> term {& term} left associative
//adding absolute difference to the parse tree right after the multiplication and devision and before the addition and subtraction based on the precedence.
TreeNode* AbsDifference(CompilerInfo* pci, ParseInfo* ppi)
{

    pci->debug_file.Out("Start Absolute Difference");
    //adding absolute difference node to tree by adding its info to the term (the * and / node) to preserve the precedence
    ...
    while(ppi->next_token.type==ABSOLUTE_DIFFERENCE)
    {
        ...
        new_tree->child[0]=tree;
        Match(pci, ppi, ppi->next_token.type);
        //term is the child of this node, so the multiplication and devision is below the absolute difference, which means higher precedence
        new_tree->child[1]=Term(pci, ppi);

        tree=new_tree;
    }
    pci->debug_file.Out("End Absolute Difference");
    return tree;
}

// mathexpr -> absdiffexpr { (+|-) absdiffexpr }    left associative
TreeNode* MathExpr(CompilerInfo* pci, ParseInfo* ppi)
{
    pci->debug_file.Out("Start MathExpr");

    TreeNode* tree=AbsDifference(pci, ppi);
    ...
    while(ppi->next_token.type==PLUS || ppi->next_token.type==MINUS)
    {
        ...
        new_tree->child[0]=tree;
        Match(pci, ppi, ppi->next_token.type);
        //absolute difference is the child of addition and subtraction so it has higher precedence
        new_tree->child[1]=AbsDifference(pci, ppi);
...
//adding absolute difference enum, if current node in parse tree is absolute difference return the result of the function above
int Evaluate(TreeNode* node, SymbolTable* symbol_table, int* variables)
{
    ...
    if(node->oper==ABSOLUTE_DIFFERENCE) return AbsDiff(a,b);
